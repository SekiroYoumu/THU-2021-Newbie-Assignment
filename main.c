...
int main()
{   
    system("mode con cols=128 lines=47"); //初始化界面大小为长128，宽45！
    initialize(); //程序初始化
    main_menu();  //加载菜单
    return 0;
}
...
// 21.5.27 v1.00 函数接口完工，将所有函数的接口改变成整型数“唯一id”
// 21.5.27 v1.10 完整的平台架构建成。
// 21.5.27 v1.20 修复了基因名中包含“\”或“/”等Windows创建文件不支持的符号导致的bug。加长基因信息结构体。第一次Release。
// 21.5.28 v1.30 优化界面显示效果与内容，解决未清空缓冲区导致的菜单选择出错问题。增加了几个菜单的返回上级菜单方式。增加了进入程序时可选的低权限访客模式。
// 21.5.28 v1.31 添加了第一部分的代码注释。添加了程序图标（by Sirius_Neo，致谢)。
// 21.5.28 v1.32 全部代码注释初步加注完成。做了一些可能无法被察觉的优化与修复。第二次Release与小范围测试（炸酒馆）。
// 21.5.29 v1.32 上传了Github。
